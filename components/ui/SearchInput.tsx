...
export const SearchInput = React.forwardRef<HTMLInputElement, SearchInputProps>(
  (
    ...
    const clearIcon = (
      <div
        onClick={handleClear}
        className="hover:bg-blue-bayoux/10 rounded p-1 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-dodger-blue focus:ring-opacity-50"
        title="Clear search"
      >
        <svg
          width="24"
    ...
    return (
      <div
        className={cn(
          'flex h-12 w-full items-center justify-center gap-2 rounded-lg border border-dodger-blue px-4 py-3',
          className
        )}
      >
        <div className="flex flex-shrink-0 items-center">{searchIcon}</div>
        ...
        <input
          ...
          className={cn(
            'min-w-0 flex-1 overflow-hidden text-ellipsis bg-transparent text-sm font-normal text-polo-blue outline-none',
        ...
        {inputValue && inputValue.length > 0 && (
          <div className="flex flex-shrink-0 items-center">{clearIcon}</div>
        )}
      </div>
    )
  }
